...
## 2026-07-09 — LOOP-010 naming cleanup hardening (adversarial probe findings)

- Confirmed: an adversarial safety probe of the LOOP-010 commit (`f5473b2`) independently reproduced 248/9 and confirmed the core safety design (server-recomputed-renames-only, dry-run defaults in model and route, per-entry apply-time realpath containment, prefix-bypass refuted, symlink-rename semantics, audit persistence, UI confirm gating). It reported three defects, all fixed same-day in `webapp/app.py` `_apply_naming_cleanup`:
- Fixed (MEDIUM — inert source-safety net): when `source_sets` is empty the per-entry containment refusal could never fire. Non-dry-run apply now fails closed with 400 when no source roots are configured (dry-run and preview remain available), and — defense in depth — with 400 when the destination realpath overlaps a configured source root in either direction (dest inside a source tree, or a source tree nested inside dest). Tests: `test_apply_refuses_when_no_source_sets_configured` (dry-run 200 / real 400), `test_apply_refuses_dest_inside_source_root`, `test_apply_refuses_dest_containing_source_root`; per-entry check still covered via `test_apply_skips_symlink_entry_resolving_into_source`.
- Fixed (LOW-MEDIUM — check-then-rename clobber window): bare `os.rename` atomically replaces an existing file target on POSIX, and the `os.path.exists` pre-check missed broken symlinks. Files and symlinks now rename via atomic no-replace `os.link` (+ `follow_symlinks=False` for symlinks) then `os.unlink`, with `FileExistsError` → skipped and rollback of the new link if the old-name unlink fails; the pre-check uses `os.path.lexists`. Directories keep pre-check + POSIX `ENOTEMPTY`; the only residual window is an empty-dir target created between pre-check and rename, and the doc's "never clobbers" wording was corrected to state exactly that. Tests: `test_apply_race_created_target_hits_eexist_skip` (mocked lexists simulates the race; pre-existing file content survives), `test_apply_skips_when_target_is_broken_symlink`.
- Fixed (LOW — preview/apply is_dir divergence): preview classified entries with `entry.is_dir(follow_symlinks=False)` but apply used `os.path.isdir` (follows symlinks), so a symlink-to-dir got differing canonical names and silently skipped. Apply now uses `os.path.isdir(p) and not os.path.islink(p)` plus `lexists` for entry existence, aligning both stages. Test: `test_symlink_to_dir_preview_and_apply_agree` (preview proposal applies with status `renamed`, new name is still a symlink).
- Confirmed: full backend suite after hardening — 254 passed, 9 skipped (was 248/9; one dest-at-source-root test converted from per-entry-skip to fail-closed-400 semantics, seven tests added). Frontend unchanged; `tsc --noEmit`/`vite build`/vitest re-verified green. LOOP-010 remains Closed.

## 2026-07-09 — LOOP-010 destination-side naming cleanup

- Confirmed: shipped the naming-cleanup sub-feature that closes LOOP-010. Backend: new pure `propose_clean_name(name, is_dir)` in `hardlink_organizer.py` (reuses `generate_display_name`, preserves file extensions, neutralises separators/traversal tokens); `naming_cleanup_ops` audit table + `record_naming_cleanup_op`/`get_naming_cleanup_ops` in `engine/db.py`; models in `webapp/models.py`; three routes in `webapp/app.py` — `GET /api/destinations/{id}/naming/preview` (non-destructive), `POST /api/destinations/{id}/naming/apply` (dry-run by default), `GET /api/destinations/{id}/naming/history` (audit trail).
- Confirmed: safety posture verified against LOOP-010 hard requirements. Preview never writes. Apply defaults `dry_run=true`; only renames whose requested target equals the server-computed cleanup proposal (no arbitrary-rename tool); rejects path separators/`.`/`..`; refuses any entry resolving under a configured source set (realpath containment check, robust to macOS `/var`→`/private/var`); never clobbers an existing target; persists only real renames and errors to the audit log.
- Confirmed: Unraid/MergerFS constraint correctly explained — an in-place same-directory rename is metadata-only, does not relocate data across disks, and preserves the entry's inode so the hardlink back to the source survives. `_naming_layout_warning()` attaches an informational note for `/mnt/user` and `/srv/mergerfs` paths; the note is reassuring (safe), distinct from the hardlink-creation EXDEV warning.
- Confirmed: UI — new `NamingCleanupPanel.tsx` reached from the DestRegistry row action menu ("Clean names"). Loads preview, shows old→new per entry, blocks collisions, selection defaults to changed/non-blocked, offers "Preview selected (dry run)" then an explicit confirm-gated "Apply renames". API client + TS types extended.
- Confirmed: docs — `docs/DESTINATION_NAMING_CLEANUP.md` added (workflow, safety model, Unraid/MergerFS constraints); README capability line + More Detail link.
- Confirmed: tests — added `TestProposeCleanName` (`tests/test_hardlink_organizer.py`), `TestNamingCleanupOps` (`tests/test_engine_db.py`), and `TestNamingCleanupAPI` (`tests/test_webapp.py`, exercises real files via the route harness: preview, dry-run leaves fs+audit untouched, real rename + audit persisted, arbitrary-rename rejected, separator rejected, no-clobber, source-safety refusal, 404s). Full backend suite: 248 passed, 9 skipped (venv with `requirements*.txt`). Frontend: `tsc --noEmit` clean for changed files (one pre-existing unrelated `handlers.ts` error remains), `vite build` clean, vitest 1 passed.
